...
def process_plate(x, y, w, h, frame, fileName, processed_ids, lock_ids):
    global first_detection, last_detection, plates_map, saved_frames
    cx = (x + x + w) // 2
    cy = (y + y + h) // 2
    id = id_tracker.getId(cx, cy, 2*w)

    if id not in lock_ids:
        lock_ids[id] = threading.Lock()
    ...
    if id not in first_detection:
        first_detection[id] = w*h
        last_detection[id] = w*h
    else:
        last_detection[id] = w*h
...
if __name__ == "__main__":
    ...
    while True:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for plate_coord in plate_coordinates:
                ...
                futures.append(executor.submit(
                    process_plate, x, y, w, h, frame, fileName, processed_ids, lock_ids))
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            futures = []
            for id in ids_to_insert:  # Example range for the inner loop
                futures.append(executor.submit(check_id, id))

        # Wait for all tasks in the inner loop to complete
        concurrent.futures.wait(futures)
